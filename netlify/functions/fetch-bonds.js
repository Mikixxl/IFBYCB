// fetch-bonds.js
// Netlify Scheduled Function — runs once per day (configured in netlify.toml).
// Scrapes worldgovernmentbonds.com for all 35 countries and stores the result
// in Netlify Blobs so bonds-table.js can serve it instantly without scraping.
//
// Manual trigger (for testing): GET /.netlify/functions/fetch-bonds
...
// Demo baseline yields — used only if live scraping fails for a country
const DEMO_YIELD = {
...
async function scrape10yYield(slug) {
  ...
  try {
    ...
    $("table tbody tr").each((_, row) => {
      ...
      if (
        label === "10 years" || label === "10 year" || label === "10y" ||
...
exports.handler = async () => {
  ...
  console.log(`[fetch-bonds] Starting daily import of ${COUNTRIES.length} countries…`);
  ...
  for (let i = 0; i < COUNTRIES.length; i += BATCH) {
    ...
    const wave = await Promise.all(
      slice.map(async (c) => {
        ...
        console.log(`  ${c.code}: ${live !== null ? live + "% (live)" : "demo fallback"}`);
        return {
          code:     c.code,
          name:     c.name,
          flag:     c.flag,
  ...
  try {
    ...
    console.log(`[fetch-bonds] Stored ${bonds.length} bonds in Netlify Blobs (${liveCount} live).`);
  } catch (blobErr) {
    console.error("[fetch-bonds] Blob write failed:", blobErr.message);
    // Don't fail the function — data was still scraped, just not cached.
  }
